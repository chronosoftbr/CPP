...

{
    ...
    while ((numBytes > 0) && (numBytes <= 64))
// warning C6386 : Buffer overrun while writing to 'this->m_buffer' : the writable size is '64' bytes, but 'm_bufferSize++' bytes might be written.
    { 
      m_buffer[m_bufferSize++] = *current++;
        numBytes--;
    }
