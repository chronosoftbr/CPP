...
/** Usage:
    ...
    string key = "key";
    string md5hmac  = hmac< MD5  >(msg, key);
    string sha1hmac = hmac< SHA1 >(msg, key);
    string sha2hmac = hmac<SHA256>(msg, key);
...
string hmac(const void* data, size_t numDataBytes, const void* key, size_t numKeyBytes)
{
